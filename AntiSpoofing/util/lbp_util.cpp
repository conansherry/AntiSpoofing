...
void ELBP_(const cv::Mat& src, cv::Mat& dst, int radius, int neighbors) {
    neighbors = max(min(neighbors, 30), 1); // set bounds...
...
template <typename _Tp>
void VARLBP_(const cv::Mat& src, cv::Mat& dst, int radius, int neighbors) {
...
void VARLBP(const cv::Mat& src, cv::Mat& dst, int radius, int neighbors) {
    switch (src.type()) {
...
cv::Mat OLBP(const cv::Mat& src) { cv::Mat dst; OLBP(src, dst); return dst; }
cv::Mat ELBP(const cv::Mat& src, int radius, int neighbors) { cv::Mat dst; ELBP(src, dst, radius, neighbors); return dst; }
cv::Mat VARLBP(const cv::Mat& src, int radius, int neighbors) { cv::Mat dst; VARLBP(src, dst, radius, neighbors); return dst; }
